    ...
    <div v-if="product" class="bg-white rounded-lg shadow overflow-hidden">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 p-6">
        ...
        <div class="flex flex-col">
          ...
          <div class="mt-1 text-sm">
            <span v-if="product.stock > 10" class="text-green-600">Trong kho còn ({{ product.stock }} )</span>
            <span v-else-if="product.stock > 0" class="text-yellow-600">Chỉ còn {{ product.stock }} trong kho!</span>
          ...
          <div class="mt-6 flex items-center">
            <label for="quantity" class="mr-4 text-sm font-medium">Số lượng:</label>
            <div class="flex items-center border border-gray-300 rounded-md">
              <button @click="quantity > 1 ? quantity-- : null" class="px-3 py-1 text-gray-600 hover:bg-gray-100"
                :disabled="quantity <= 1">
                -
              </button>
              <input id="quantity" v-model.number="quantity" type="number" min="1" :max="product.stock"
                class="w-12 text-center border-none focus:ring-0" />
              <button @click="quantity < product.stock ? quantity++ : null"
                class="px-3 py-1 text-gray-600 hover:bg-gray-100" :disabled="quantity >= product.stock">
                +
              </button>
            </div>
          </div>
          ...
          <!-- Add to Cart Button -->
          <button @click="addToCart" class="mt-6 btn btn-primary" :disabled="product.stock === 0">
      ...
      <div class="border-t mt-8">
        <div class="flex border-b">
          <button @click="activeTab = 'description'" class="px-4 py-3 font-medium text-sm"
            :class="activeTab === 'description' ? 'border-b-2 border-green-600 text-green-600' : 'text-gray-600 hover:text-green-600'">
            Chi tiết
          </button>
          <button @click="activeTab = 'reviews'" class="px-4 py-3 font-medium text-sm"
        ...
        <div class="p-6">
          ...
          <div v-if="activeTab === 'reviews'">
            <div v-if="isLoggedIn" class="bg-gray-50 p-4 rounded-lg mb-6">
              <h3 class="text-lg font-medium mb-4">Viết Review</h3>
              <div class="mb-4">
                <label class="form-label">Rating</label>
                <div class="flex">
                  <button v-for="star in 5" :key="star" @click="newReview.rating = star" class="focus:outline-none">
                    <StarIcon class="h-6 w-6" :class="star <= newReview.rating ? 'text-yellow-400' : 'text-gray-300'" />
                  </button>
                </div>
              </div>
              <div class="mb-4">
                <label for="review-comment" class="form-label">Comment</label>
                <textarea id="review-comment" v-model="newReview.comment" rows="3" class="form-input"
            ...
            <div v-if="product.reviews.length > 0" class="space-y-4">
              <div v-for="(review, index) in product.reviews" :key="index" class="border-b pb-4 last:border-b-0">
                <div class="flex justify-between items-start">
                  <div>
                    ...
                    <div class="flex mt-1">
                      <StarIcon v-for="star in 5" :key="star" class="h-4 w-4"
                ...
                <button v-if="review.userId === currentUser?.id" @click="deleteReview(index)"
